...
@pytest.mark.parametrize("expected_location_id", ["am2", "nj3", "br", "kz", "ca"])
def test_locations_list(expected_location_id, public_api_client):
    """The test checks that the specified locations are available for creating resources in them"""
    locations_response = public_api_client.get_locations_list()
    received_location_ids = [location["id"] for location in locations_response["locations"]]
    assert expected_location_id in received_location_ids, f"Expected location '{expected_location_id}' not found"
...
def test_images_list(expected_image, public_api_client):
    ...
    received_images_ids = [images["id"] for images in images_response["images"]]
    assert expected_image in received_images_ids, f"Expected image '{expected_image}' not found"
