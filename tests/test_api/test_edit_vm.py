...
def test_edit_vm_configuration(public_api_client, cpu, ram_mb,
                               expected_cpu, expected_ram_mb,
                               expected_error_code, expected_error_message):
...
@pytest.mark.parametrize("name, size_mb, expected_name, expected_size_mb,"
                         "expected_error_code, expected_error_message",
                         [
                             ('valid', '30720', 'valid', 30720, None, None),
                             ('non_10', '22528', None, None, 400,
                              'Volume size is not supported'),
                             ('to_small', '2048', None, None, 400,
                              'The volume is too small'),
                             ('to_big', '2048000', None, None, 400,
                              'Volume size limit exceeded'),
                             ('zero', '0', None, None, 400,
                              'Volume size is not supported')
                         ])
def test_add_vm_volume(public_api_client, name, size_mb,
                       expected_name, expected_size_mb,
                       expected_error_code, expected_error_message):
    """Test checks the add volume request for a virtual machine
    ...
    add_volume_response = public_api_client.add_volume(vm_id,
                                                       name,
                                                       size_mb,
                                                       expected_error_code,
    ...
    if expected_error_code is not None:
        assert add_volume_response.status_code == expected_error_code, \
            'Expected error code does not match'
        ...
        PublicAPIFuncs.wait_until_task_completed(public_api_client, task_id_value)
        vol_inf = public_api_client.get_vm_by_id(vm_id)['server']['volumes'][1]
        assert vol_inf['name'] == expected_name
        assert vol_inf['size_mb'] == expected_size_mb
    public_api_client.delete_vm(vm_id)
    public_api_client.get_vm_by_id(vm_id, 404)
